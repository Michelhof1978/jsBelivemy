  ...
  state = {//State est un object React qui permet de gérer les données, le but du state est de regrouper toutes les informations ds un endroit
  eleves: [//On crée un tableau d'objets et ds chaque tableau, il y aura un object javascript
    ...
    buttonClickedHandler = () => {//Fonction pour le bouton ci dessous, grâce à elle, l événement se déclanchera lors du clic sur bouton, s il n'y avait pas cette fonction le jsx lancera le js qui est à l intérieur de lui automatiquement, ce qui poserait problème
      console.log('clic')//On peut nommer la fonction comme on veut mais par convention, on ajoutera à la fin du nom de la fonction Handler
    }
  ...
  <h1>BIENVENUE MIMICHE</h1>//On appele ce code du sucre syntaxique = JSX, pour écrire plus facilement du Js
   ...
   <button onClick = {this.buttonClickedHandler}> Mon bouton </button>//Comme ce n'est pas du js mais du jsx et que l on veut créer un événement, il suffir juste de mettre la majuscule à onClick, comme c'est une class, on ajoute this pour appeler la fonction buttonClickedHandler
 ...
 //Pour appeler buttonClickedHandler et pour que ça fonctionne, on ne mets pas de parenthèse sinon 
  <Eleve nom = {this.state.eleves[0].nom} moyenne = {this.state.eleves[0].moyenne}>
    ...
    <Eleve nom = {this.state.eleves[1].nom} moyenne = {this.state.eleves[1].moyenne}>
