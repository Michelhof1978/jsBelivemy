...
Dans cet exemple ci dessus, il s'agit d'une utilisation d'un composant personnalisé nommé "Eleve" avec des propriétés spécifiques.
Le composant "Eleve" est appelé avec des propriétés passées en paramètres, dans ce cas "nom", "moyenne" et "clic". Les propriétés "nom" et "moyenne" sont définies en utilisant les données du deuxième élément du tableau "eleves" (c'est-à-dire l'objet représentant le deuxième élève). La propriété "clic" est définie en passant une fonction fléchée qui appelle une méthode "buttonClickedHandler" avec le paramètre "nicolas".

Le texte à afficher dans le composant "Eleve" est défini à l'aide de la propriété "citation", qui est également extraite du deuxième élément du tableau "eleves".

En utilisant le composant personnalisé "Eleve", les développeurs peuvent créer des composants réutilisables et maintenables, qui peuvent être utilisés avec différentes données et propriétés pour afficher des éléments dans une interface utilisateur.