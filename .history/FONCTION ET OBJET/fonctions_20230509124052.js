...
Cette ligne de code ci dessus utilise la méthode `filter()` pour créer un nouveau tableau appelé `filteredArr` à partir d'un tableau existant `arr`. 

Le tableau `filteredArr` contiendra uniquement les éléments de `arr` qui sont supérieurs à 2. 

La fonction fléchée (`num => num > 2`) est utilisée comme critère de filtrage pour déterminer quels éléments doivent être inclus dans le tableau `filteredArr`. 

En d'autres termes, pour chaque élément `num` de `arr`, la fonction fléchée vérifie si `num` est supérieur à 2. Si c'est le cas, `num` est inclus dans `filteredArr`. Si ce n'est pas le cas, `num` est exclu de `filteredArr`.
