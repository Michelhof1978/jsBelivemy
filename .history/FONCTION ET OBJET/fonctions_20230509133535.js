...
console.log(newArr);
-------------------------------------------------------------------------------------------------------
//FONCTION D ORDRE SUPERIEURE, plus simple à faire pour pouvoir moduler le code plus facilement
const rawArr = [5,9,200, 345, 77]
...
console.log(arrSup100);//Resultat identique du code ci dessus
----------------------------------------------------------------------------------------------------
//FONCTION CALLBACK: une fonction qui appelle une autre fonction
const arr = [1,2,3,4,5,6];
const mapedArr = arr.map(x => x + 10);//Retourne un nouveau tableau grâce à map, il remplacera le [1,2,3,4,5,6] en ajoutant 10 à chaque données du tableau. 

console.log(mapedArr);//Résultat: [11,12,13,14,15,16]

const filteredArr = arr.filter(num => num > 2)//La méthode filter va retourner un nouveau tableau comme la méthode map
// Cette ligne de code ci dessus utilise la méthode `filter()` pour créer un nouveau tableau appelé `filteredArr` à
// partir d'un tableau existant `arr`. 
// Le tableau `filteredArr` contiendra uniquement les éléments de `arr` qui sont supérieurs à 2. 
// La fonction fléchée (`num => num > 2`) est utilisée comme critère de filtrage pour déterminer quels éléments 
// doivent être inclus dans le tableau `filteredArr`. 
// En d'autres termes, pour chaque élément `num` de `arr`, la fonction fléchée vérifie si `num` est supérieur à 2. 
...
Ce code suppose qu'il y a un objet appelé "pays" qui a deux propriétés: "nom" et "pop". La première ligne déstructure l'objet "pays" pour extraire les valeurs des propriétés "nom" et "pop" et les stocker dans des variables du même nom. 

La deuxième ligne crée une fonction fléchée appelée "data" qui prend un objet en entrée et retourne la valeur de sa propriété "nom". Cependant, cette fonction utilise une syntaxe raccourcie pour extraire la propriété "nom" de l'objet passé en paramètre.

La troisième ligne appelle la fonction "data" en passant l'objet "pays" comme argument, et la valeur retournée est affichée dans la console. En résumé, ce code extrait la valeur de la propriété "nom" de l'objet "pays" en utilisant une fonction fléchée raccourcie.
-------------------------------------------------------------------------------------------------