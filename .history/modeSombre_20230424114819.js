...
------------------------------------------------------------
  let button = document.querySelector('#mode');//On selectionne l id bouton
let span   = document.querySelector('span');//On selectionne le span
...
button.addEventListener('click', () => {//Détécter l événement click
  if(document.body.classList.contains('dark')) {//fait appel à la classe dark du css avec classList et va détécter si c'est sombre ou clair
    document.body.className = '';//Retirer toutes les classes de l'élément body
    span.textContent = 'Thème sombre';
    localStorage.setItem('theme', 'clair');//La méthode localstorage.setitem('theme', 'clair') est utilisée pour stocker une chaîne de caractères 'clair' dans la clé 'theme' du stockage local. Pour l'utiliser, il suffit d'appeler la méthode en passant les arguments appropriés.
  }
  else {
    modeSombre();
  }
