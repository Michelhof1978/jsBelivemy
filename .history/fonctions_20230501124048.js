...
let PlusGrandQue10 = prices.filter(function(p){//On stocke notre fonction anonyme la variable p et on utilise pour tous les prix(prices) la fonction filter pour filtrer, trier les prix
  ...
  console.log (letPlusGrandQue10);
  Ce code crée un tableau price contenant des prix, puis utilise la méthode filter() pour créer un nouveau tableau PlusGrandQue10 qui ne contient que les prix supérieurs à 10. Enfin, il affiche le nouveau tableau PlusGrandQue10 dans la console.

La méthode filter() garde uniquement les éléments qui réussissent le test (dans ce cas, le test est de savoir si l'élément est supérieur à 10), et les renvoie dans un nouveau tableau.
------------------------------------------------------------------------------------
