// Une promesse en JavaScript est un objet qui représente la valeur éventuelle (ou l'erreur) qu'une opération asynchrone pourrait retourner dans le futur. 

// Prenons l'exemple d'une requête AJAX pour récupérer des données d'un serveur distant. Cette opération est asynchrone, car elle peut prendre un certain temps pour récupérer les données, et pendant ce temps-là, le reste de votre code continuera de s'exécuter.

// Au lieu de bloquer l'exécution du code en attendant que les données soient récupérées, une promesse est renvoyée immédiatement, ce qui permet au reste du code de continuer à s'exécuter. La promesse contient une fonction callback qui sera exécutée une fois que les données seront récupérées avec succès.

// Si les données sont récupérées avec succès, la promesse est résolue avec la valeur retournée. Si une erreur se produit pendant la récupération des données, la promesse est rejetée avec un objet d'erreur.

// Les promesses sont un moyen pratique et élégant de gérer des opérations asynchrones en JavaScript, car elles permettent d'éviter les problèmes de callback hell et de rendre le code plus lisible et facile à comprendre.
